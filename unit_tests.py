...
class TestGraphLSTMNet(unittest.TestCase):
    ...
    def test_call_uninodal_notf(self):
        ...
        try:
            unet.call(([cell_input]), ((cell_state_m, cell_state_h),))
        except IndexError:
            pass
        else:
            print "22 GraphLSTMNet with one node did not complain about index 1"
...
def test_init_GraphLSTMNet():
    ...
    try:
        ...
        print "00 GraphLSTMNet did not raise TypeError when init'd with something else than a nx.Graph"

    try:
        rci.GraphLSTMNet(None)
    except ValueError:
        pass
    else:
        print "01 GraphLSTMNet did not raise ValueError when init'd with None as graph"
    ...
    try:
        ...
        print "02 GraphLSTMNet did not raise TypeError when constructed with empty constructor"
    ...
    try:
        ...
        print "03 GraphLSTMNet._cell() did not raise KeyError for non-existing node"
    ...
    try:
        ...
        print "04 GraphLSTMNet._cell() did not raise KeyError for node without cell"
...
def test_call_uninodal_GraphLSTMNet_notf():
    ...
    try:
        gnet.call(([cell_input]), ((cell_state_m, cell_state_h),))
    except IndexError:
        pass
    else:
        print "22 GraphLSTMNet with one node did not complain about index 1"
